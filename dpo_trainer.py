...
class DPOTrainer(Trainer):
    ...
    def dpo_loss(
        ...
        if self.loss_type == "sigmoid":
            ...
            # 正则化项: λ * max(0, log(π_ref(y_w|x)/π_θ(y_w|x)))
            regularization_term = lambda_ * torch.clamp(reference_chosen_logps - policy_chosen_logps, min=0)

            # 核心表达式：β * (log_ratio_diff) - regularization_term
            core_expression = self.beta * (chosen_logratios - rejected_logratios) - regularization_term

            # 应用 sigmoid 函数并计算负对数似然损失
            losses = -torch.log(torch.sigmoid(core_expression))
